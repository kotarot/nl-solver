...
int main(int argc, char *argv[]){
	...
	// ファイルの読み込み
	if(argc != 3){
		cout << "Usage: ./solver.exe InputFile OutputFile" << endl;
		return 0;
	}
	initialize(argv[1]); // 問題盤の生成
	printBoard(); // 問題盤の表示
	
	int pid[P_MAX]; // 子プロセスIDの格納
	int status;
	
	// 任意のプロセスが解を出力したら，trueに
	bool finished = false;
	...
	if(i==P_MAX){ // 親プロセス（監視役）
		
		time_t t1, t2;
		t1 = time(NULL);
	
		for(int j=0;j<P_MAX;j++){
			int c_pid = wait(&status); // 子プロセスの終了待ち
			// if文の第1引数: 正常終了判定
			// if文の第2引数: 正解既導出判定
			// if文の第3引数: 正解導出判定
			if(WIFEXITED(status) && !finished && WEXITSTATUS(status)==0){
				finished = true; // 正解既導出フラグ
				
				// 正解導出プロセスの探索
				int idx;
				for(idx=0;idx<P_MAX;idx++){
					if(pid[idx]==c_pid) break;
				}
				
				// 活動プロセスに割込み終了
				for(int k=0;k<P_MAX;k++){
					if(kill(pid[k],0)==0) kill(pid[k],2);
				}
				...
				ostringstream os;
				os << idx;
				string oldname = "answer_" + os.str() + ".txt";
				// 新しいファイル名（提出用正解ファイル）
				//string newname = "answer.txt";
				
				// ファイル名の変更
				if(rename(oldname.c_str(),argv[2])==0){
					cout << "File renamed!" << endl;
				}
			}
		}
		cout << "Complete!(Parent)" << endl;
		
		t2 = time(NULL);
		cout << "Time[sec]: " << (t2-t1) << endl;
	}
	else if(pid[i]==0){ // 子プロセス
		if(proc(i)){ exit(0); } // (終了)正解導出
		else{ exit(1); } // (終了)正解導出不可
	}
	else{
...
bool proc(int idx){
	...
	// 乱数の初期化
	srand(((unsigned int)time(NULL))*(idx+1));
	// ペナルティの初期化
	penalty_T = 0;
	penalty_C = 0;
	...
	for(int m=2;m<=O_LOOP;m++){ // 外ループ
		...
		for(int n=1;n<=I_LOOP;n++){ // 内ループ
			...
			if((board->line(id))->isIntermediateUsed()){ // 中間ポート利用経路探索
				...
				success = routing(id);
				if(!success){
					cout << "Cannot solve!!(" << idx << ")" << endl; // 失敗したらプログラム終了
					delete board;
					return false;
				}
			}
	...
	// 解導出できなかった場合
	if(!isFinished()){
		cout << "Cannot fight!!(" << idx << ")" << endl; // プログラム終了
		delete board;
		return false;
		//for(int i=1;i<=board->getLineNum();i++){
		//	printLine(i);
