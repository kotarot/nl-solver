...
bool routing(int trgt_line_id/*, bool debug_option*/, int penalty_T, int penalty_C, int penalty_V, int *output){
	...
	Board _board;
	_board.init(size_x, size_y, size_z, line_num, via_num);
	...
	if(isInserted_1(start_x,start_y-1,start_z, board)){
		...
		qu[qu_index] = trgt; qu_index++; //qu.push(trgt);
	}
	// 東方向を探索
	...
	while (qu_index != 0) { //while(!qu.empty()){

		//Search trgt = qu.front();
		//qu.pop();
		qu_index--;
		Search trgt = qu[qu_index];
		...
		if(trgt.d!=NORTH && isInserted_1(trgt.x,trgt.y-1,start_z, board)){
			...
			qu[qu_index] = next; qu_index++; //qu.push(next);
		}
		// 東方向
